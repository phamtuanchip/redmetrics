module Redmetrics
  module ProjectCalculations
    ...
    def calculate_risk(level, to_date, project)
      ...
      raise Redmetrics::Exception, l(:'redmetrics.redmetrics_risk_tracker_not_available') if risk_tracker.nil?
      cond = ARCondition.new
    ...
    def calculate_incidence(level, to_date, project)
      ...
      raise Redmetrics::Exception, l(:'redmetrics.redmetrics_incidence_tracker_not_available') if incidence_tracker.nil?
      cond = ARCondition.new
      cond << ['created_on < ?', to_date]
    ...
    def calculate_effort_general budget_type, project_metrics
      ...
      return effort.round(2).to_s + " #{l('redmetrics.label_hours')}"
    end

    def calculate_effort_remaining_general project_metrics
      User.roles.each do |role|
        project_metrics["effort_remaining_#{role.underscore}"] = project_metrics["Esfuerzo actual #{l('redmetrics.label_' + role)}"].nil? ? "-- #{l('redmetrics.label_hours')}" :
                                                    (project_metrics["Esfuerzo actual #{l('redmetrics.label_' + role)}"].to_f -
                                                    project_metrics['effort_done_' + role.underscore].to_f).round(2).to_s + " #{l('redmetrics.label_hours')}"
      end
      return project_metrics
    end
    ...
    def calculate_effort_remaining project_metrics
      ...
      return effort_remaining.round(2).to_s + " #{l('redmetrics.label_hours')}"
    end
    ...
    def calculate_no_approval_open project
      ### Suma de "no conformidades" en estado "Nueva" o "Validada"
        ...
        raise Redmetrics::Exception, l(:'redmetrics.redmetrics_qa_tracker_not_available') if risks.nil?
        cond = ARCondition.new
        if @informe
          cond << ['created_on BETWEEN ? AND ?', 0, @date]
        end
  ...
  #   Esfuerzo en nuevos requisitos CMMI_Solicitudes de cambio Aceptada, Resuelta o Cerrada
        ...
        raise Redmetrics::Exception, l(:'redmetrics.redmetrics_change_tracker_not_available') if tracker_requests_change.nil?
        cond = ARCondition.new
        if @informe
          cond << ['created_on BETWEEN ? AND ?', 0, @date]
        end
